  ...
  'use strict';
    ...
    if ($('.pagepiling').length > 0) {
      ...
      $('.pagepiling').pagepiling({
        ...
        afterLoad: function(anchorLink, index) {
            /* // Temporarily commented out theme switching logic
            var activeSection = $('.pp-scrollable').eq(index - 1);
            if (activeSection.hasClass('section-white')) {
        ...
        onLeave: function(index, nextIndex, direction) {
            // Optional: Add logic for section leave events if needed
        }
      });
      ...
      /* Add Arrows On Navigation (Restoring original method) */
      if ($('#pp-nav').length > 0) {
      ...
      /* Scroll Navbar Into Sections (If needed - might conflict with afterLoad logic) */
      /*
	...
	$(document).ready(function() {
		...
		function updateSteps() {
			...
			$('.process-step[data-step="' + currentStep + '"]').addClass('active');

			// Update button states
			$('#process-prev').prop('disabled', currentStep === 1);
			$('#process-next').prop('disabled', currentStep === totalSteps);
		}
